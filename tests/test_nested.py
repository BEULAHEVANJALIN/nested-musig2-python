...
def test_branch_level_reconstructs_parent_cache_from_child_and_siblings():
    alice = LeafSigner.generate("Alice")
    bob = LeafSigner.generate("Bob")
    level = NestedBranchLevel(
        child_pubkey=alice.pubkey,
        sibling_pubkeys=[bob.pubkey]
    )
    ...
    print("Branch levels rebuild parent MuSig2 cache from child and sibling keys")
...
def test_transcript_root_leaf_derives_factors_locally():
    ...
    branch_witness = NestedBranchWitness(
        levels=[NestedBranchLevel(child_pubkey=alice.pubkey, sibling_pubkeys=[bob.pubkey])],
...
def test_transcript_nested_leaf_derives_factors_locally():
    ...
    branch_witness = NestedBranchWitness(
        levels=[
            NestedBranchLevel(child_pubkey=group_ab.cache.agg_pk, sibling_pubkeys=[carol.pubkey]),
            NestedBranchLevel(child_pubkey=alice.pubkey, sibling_pubkeys=[bob.pubkey]),
        ],
...
def test_transcript_deep_leaf_derives_factors_locally():
    ...
    branch_witness = NestedBranchWitness(
        levels=[
            NestedBranchLevel(child_pubkey=group_top.cache.agg_pk, sibling_pubkeys=[eve.pubkey]),
            NestedBranchLevel(child_pubkey=group_l.cache.agg_pk, sibling_pubkeys=[carol.pubkey]),
            NestedBranchLevel(child_pubkey=alice.pubkey, sibling_pubkeys=[bob.pubkey]),
        ],
        nested_nonce_bindings=[
...
def test_nested_partial_verifier_rejects_wrong_session():
    ...
    bad_branch_witness = NestedBranchWitness(
        levels=[
            NestedBranchLevel(child_pubkey=group_ab_2.cache.agg_pk, sibling_pubkeys=[carol_2.pubkey]),
            NestedBranchLevel(child_pubkey=alice_2.pubkey, sibling_pubkeys=[bob_2.pubkey]),
        ],
...
def test_branch_level_rejects_duplicate_child_in_siblings():
    alice = LeafSigner.generate("Alice")
    bob = LeafSigner.generate("Bob")
    try:
        NestedBranchLevel(
            child_pubkey=alice.pubkey,
            sibling_pubkeys=[alice.pubkey, bob.pubkey],
        )
        assert False, "Expected duplicate child key in sibling set to raise ValueError"
    except ValueError as e:
        assert "must not appear in sibling" in str(e)
    print("Branch levels reject duplicate child keys in sibling sets")
...
def test_transcript_rejects_wrong_subgroup_key_in_path():
    ...
    bad_branch_witness = NestedBranchWitness(
        levels=[
            NestedBranchLevel(child_pubkey=group_top.cache.agg_pk, sibling_pubkeys=[eve.pubkey]),
            NestedBranchLevel(child_pubkey=carol.pubkey, sibling_pubkeys=[group_l.cache.agg_pk]),
            NestedBranchLevel(child_pubkey=alice.pubkey, sibling_pubkeys=[bob.pubkey]),
        ],
        nested_nonce_bindings=[
            nested_bindings["Group_Top"],
            ...
            branch_witness = NestedBranchWitness(
                levels=path_levels + [
                    NestedBranchLevel(
                        ...
                        sibling_pubkeys=[pk for pk in path_caches[-1].sorted_pks if pk != member.pubkey],
                    )
                ],
        ...
        for child in member.members:
            if isinstance(child, LeafSigner):
                ...
                collect_signatures(
                    ...
                    path_levels + [
                        NestedBranchLevel(
                            child_pubkey=child.cache.agg_pk,
                            sibling_pubkeys=[pk for pk in path_caches[-1].sorted_pks if pk != child.cache.agg_pk],
                        )
                    ],
...
def test_branch_witness_rejects_wrong_nested_binding_count():
    alice = LeafSigner.generate("Alice")
    bob = LeafSigner.generate("Bob")
    carol = LeafSigner.generate("Carol")
    group_ab = NestedGroup("Group_AB", [alice, bob])
    msg = b"bad transcript"
    _, _, nested_bindings = _make_session([group_ab, carol], msg)
    try:
        NestedBranchWitness(
            levels=[
                NestedBranchLevel(child_pubkey=group_ab.cache.agg_pk, sibling_pubkeys=[carol.pubkey]),
        ...
        assert False, "Expected wrong nested binding count to raise ValueError"
    except ValueError as e:
        assert "nested levels" in str(e)
    print("Branch witnesses reject inconsistent nested binding counts")
...
if __name__ == "__main__":
    ...
    test_branch_level_reconstructs_parent_cache_from_child_and_siblings()
    test_many_sessions()
    ...
    test_branch_level_rejects_duplicate_child_in_siblings()
    ...
    test_branch_witness_rejects_wrong_nested_binding_count()
